...
import com.mykelalexander.zombiebarricade.uiutils.CivilianManager;
import com.mykelalexander.zombiebarricade.uiutils.ComboManager;
...
public class World 
{
	...
	//GAME STATS
	public int score;
	public CivilianManager civilians;
	...
	public World( Level level )
	{
		...
		zombieManager = new ZombieManager(level.getLevelProperties(), this);
		...
		if(this.level.hasCivilians)
			civilians = new CivilianManager(this.level.civilians);
	}
				...
				if(zPos.xPos == oPos.xPos && zPos.yPos <= oPos.yPos + TILE_HEIGHT/2 && zPos.yPos - zPos.yVel >= oPos.yPos + TILE_HEIGHT/2 )
				{
					...
					if(object instanceof BarricadeBlock)
					{
						...
						if(((zType == ZombieType.YELLOW && block.getBlockType() == BarricadeBlock.YELLOW_BLOCK)
								...
								|| (zType == ZombieType.BLUE && block.getBlockType() == BarricadeBlock.BLUE_BLOCK)) && !(object.getState() == ObjectState.ACTIVATING)
								&& !(object.getState() == ObjectState.USED))
						{
							comboManager.addCombo(block.activate(zombies, j));
						}
					}
