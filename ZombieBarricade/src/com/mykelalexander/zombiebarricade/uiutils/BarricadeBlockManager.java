...
public class BarricadeBlockManager 
{
	...
	public int getBlockTimer() { return (int)blockTimer; }
	...
	public void generateBlock()
	{
		...
		if(currentBlockIndex < level.blockBuffer.length)
		{
	...
	public void liftBlock(int i)
	{
		...
		case BarricadeBlock.RED_BLOCK:
			sm.loopSound(S_RED_BLOCK_LIFTED, 1, 1, 1);
			break;	
		case BarricadeBlock.BLUE_BLOCK:
			sm.loopSound(S_BLUE_BLOCK_LIFTED, 1, 1, 1);
			break;
		case BarricadeBlock.YELLOW_BLOCK:
			sm.loopSound(S_YELLOW_BLOCK_LIFTED, 1, 1, 1);
			break;
		}
	}
	...
	public void updateBlocks(float delta)
	{
		...
		for(int i = 0; i < objectLayer.size(); i++)
		{
			...
			objectLayer.get(i).update(delta);
		}
		
		nextBlock.update(delta);
		
		if(liftedBlock != null)
			liftedBlock.update(delta);
	}
